class PredictiveSearch extends SearchForm {
  ...
  setupEventListeners() {
    ...
    // this.productType.addEventListener('change', this.onLoad.bind(this));
    if(this.productType){
      this.productType.addEventListener('change', debounce((event) => {
        this.onLoad(event);
      }, 300).bind(this))
    }
  ...
  getQuery() {
    ...
    if (this.productType){
      const productType = this.productType.value.trim();
      const combinedValue = `product_type:${productType}${inputValue ? '+' + inputValue : ''}`;
      // const combinedValue = `product_type:${productType}${inputValue}`;
  ...
  onFormSubmit(event) {
    ...
    const baseUrl = "?type=product&options%5Bprefix%5D=last&options%5Bunavailable_products%5D=last";
    const url = form.action+`${baseUrl}&q=${query}`;    
  ...
  getSearchResults(searchTerm) {
    ...
    if (this.cachedResults[queryKey]) {
      ...
      fetch(    
        `/search?type=product&options%5Bprefix%5D=last&options%5Bunavailable_products%5D=last&q=${encodeURI(
          searchTerm
        )}&view=js`      
      )    
      .then((response) => {
